...
func NewPrometheusExporter(metricsProvider *MetricsProvider, siteId string) *PrometheusExporter {
	exporter := &PrometheusExporter{
		...
		// Backhaul metrics
		backhaulSwitchPortStatus: promauto.NewGaugeVec(prometheus.GaugeOpts{
			Name: "backhaul_switch_port_status",
			Help: "Backhaul switch port status (1 = up, 0 = down)",
		}, []string{"unit", "site"}),
		backhaulSwitchPortSpeed: promauto.NewGaugeVec(prometheus.GaugeOpts{
			Name: "backhaul_switch_port_speed",
			Help: "Backhaul switch port speed in Mbps",
		}, []string{"unit", "site"}),
		backhaulSwitchPortPower: promauto.NewGaugeVec(prometheus.GaugeOpts{
		...
		solarSwitchPortSpeed: promauto.NewGaugeVec(prometheus.GaugeOpts{
			Name: "solar_switch_port_speed",
			Help: "Solar controller switch port speed in Mbps",
		}, []string{"unit", "site"}),
		solarSwitchPortPower: promauto.NewGaugeVec(prometheus.GaugeOpts{
		...
		nodeSwitchPortSpeed: promauto.NewGaugeVec(prometheus.GaugeOpts{
			Name: "node_switch_port_speed",
			Help: "Node switch port speed in Mbps",
		}, []string{"unit", "site"}),
		nodeSwitchPortPower: promauto.NewGaugeVec(prometheus.GaugeOpts{
...
func (e *PrometheusExporter) CollectMetrics() error {
	...
	if e.metricsProvider.GetPortStatus(PORT_BACKHAUL) {
		...
		e.backhaulLatency.WithLabelValues("ms", e.siteId).Set(0)
		e.backhaulSpeed.WithLabelValues("mbps", e.siteId).Set(0)
		e.switchPortStatus.WithLabelValues("status", e.siteId).Set(0)
		e.switchPortSpeed.WithLabelValues("mbps", e.siteId).Set(0)
	...
	if e.metricsProvider.GetPortStatus(PORT_BACKHAUL) {
		...
		e.backhaulSwitchPortPower.WithLabelValues("watts", e.siteId).Set(metrics.Backhaul.SwitchPortPower)
	} else {
	...
	if e.metricsProvider.GetPortStatus(PORT_NODE) {
		...
		e.nodeSwitchPortPower.WithLabelValues("watts", e.siteId).Set(0)
	}
	...
	if !e.metricsProvider.GetPortStatus(PORT_SOLAR_CONTROLLER) {
		e.solarPanelVoltage.WithLabelValues("volts", e.siteId).Set(0)
		e.solarPanelCurrent.WithLabelValues("amps", e.siteId).Set(0)
		e.solarPanelPower.WithLabelValues("watts", e.siteId).Set(0)
		e.batteryChargePercentage.WithLabelValues("percent", e.siteId).Set(0)
		voltage = 0
	}
