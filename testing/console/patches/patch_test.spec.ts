...
test('apply patch to generated test', async () => {
  try {
    ...
    const importStatement =
      "import { TEST_USER_EMAIL, TEST_USER_PASSWORD, LOGIN_URL } from '../constants';\nimport { faker } from '@faker-js/faker';\nimport { selectRandomOption } from '../utils';\n";
    ...
    const patchedContent = contentWithImport
      ...
      .replace(
        ...
        `await page.getByRole('textbox', { name: 'PRICE' }).fill('${faker.number.int({ min: 1000, max: 2000 }).toString()}');`,
      )
      .replace(
        /await page\.getByRole\('textbox', { name: 'DATA LIMIT' }\)\.fill\('.*'\);/g,
        `await page.getByRole('textbox', { name: 'DATA LIMIT' }).fill('${faker.number.int({ min: 10, max: 20 }).toString()}');`,
      )
      .replace(
        / await page\.getByRole\('option', { name: '.*' }\)\.click\(\);/g,
      ...
      .replace(
        /await page\.getByRole\('option', { name: '.*' }\)\.click\(\);/g,
        `await selectRandomOption(page, page.getByRole('combobox', { name: 'SIM ICCID*' }));`,
      )
      .replace(
        /await page\s*\.getByLabel\('', { exact: true }\)\.click\(\);\s*await page\s*\.getByRole\('option', { name: 'textor - .*' }\)\s*\.click\(\);/g,
