...
int event_runtime(event_ctx_t *ctx, const event_spec_t *event,
    ...
    case EVT_START_UES:
        ...
        if (rc == ULAB_OK) rc = runtime_start_ues(ctx->runtime, ctx->world,
            &res, err);
        selector_result_free(&res);
        return rc;
    case EVT_WAIT_UES_ATTACHED:
        rc = selector_resolve_ues(ctx->world, &event->ues, &res, err);
        if (rc == ULAB_OK) rc = runtime_wait_ues_attached(ctx->runtime,
            ctx->world, &res, err);
        selector_result_free(&res);
        return rc;
    case EVT_RESTART_NODES:
        rc = selector_resolve_nodes(ctx->world, &event->nodes, &res, err);
        if (rc == ULAB_OK) rc = runtime_restart_nodes(ctx->runtime,
            ctx->world, &res, err);
        selector_result_free(&res);
        return rc;
    case EVT_WAIT_NODES_READY:
        rc = selector_resolve_nodes(ctx->world, &event->nodes, &res, err);
        if (rc == ULAB_OK) rc = runtime_wait_nodes_ready(ctx->runtime,
            ctx->world, &res, err);
        selector_result_free(&res);
        return rc;
    default:
