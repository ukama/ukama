                              ...
                              ulab_error_t *err)
{
    ...
    code = code_value && json_is_string(code_value) ? json_string_value(code_value) : "UNKNOWN";
    message = message_value && json_is_string(message_value) ? json_string_value(message_value) : "section failed";
    snprintf(err->msg, sizeof(err->msg),
...
static json_t *console_find_id(json_t *arr, const char *id)
{
    ...
    for (i = 0; i < json_array_size(arr); i++)
    {
        ...
        actual = value && json_is_string(value) ? json_string_value(value) : NULL;
                                ...
                                ulab_error_t *err)
{
    ...
    timeout_sec = timeout_env ? (unsigned int)strtoul(timeout_env, NULL, 10) : 180u;
    ...
    while (elapsed <= timeout_sec)
    {
        ...
        if (arr != NULL && json_is_array(arr))
        {
            for (i = 0; i < json_array_size(arr); i++)
            {
                it = json_array_get(arr, i);
                idv = it ? json_object_get(it, "id") : NULL;
                id = idv && json_is_string(idv) ? json_string_value(idv) : NULL;
...
static double package_json_number(json_t *obj, const char *key)
{
    ...
    return value != NULL && json_is_number(value) ? json_number_value(value) : 0.0;
}
                            ...
                            ulab_error_t *err)
{
    ...
    duration = pkg->duration_minutes > 0 ? (long long)pkg->duration_minutes : (long long)pkg->duration_days * 1440LL;
    amount = pkg->amount;
    currency = pkg->currency[0] ? pkg->currency : "USD";
    if (ulab_streq(variant, "allowance"))
        data_volume = -1;
    else if (ulab_streq(variant, "duration"))
        duration = 0;
    else if (ulab_streq(variant, "price"))
        amount = -1.0;
    else if (ulab_streq(variant, "currency"))
        currency = "";
    else
    {
        snprintf(err->msg, sizeof(err->msg),
                            ...
                            ulab_error_t *err)
{
    ...
    for (i = 0; i < json_array_size(packages); i++)
    {
        ...
        payments = payments_obj ? json_object_get(payments_obj, "payments") : NULL;
    ...
    ulab_error_t *err)
{
    ...
    for (i = 0; i < json_array_size(components); i++)
    {
        ...
        category_value = component ? json_object_get(component, "category") : NULL;
        category = category_value && json_is_string(category_value) ? json_string_value(category_value) : NULL;
        if (category != NULL &&
                                 ...
                                 ulab_error_t *err)
{
    ...
    ulab_json_escape(currency && currency[0] ? currency : (pkg->currency[0] ? pkg->currency : "USD"),
    ...
    for (i = 0; i < json_array_size(rows); i++)
    {
        ...
        for (j = 0; j < json_array_size(attributes); j++)
        {
            ...
            if (ulab_streq(key, "name"))
                row->has_name = 1;
            else if (ulab_streq(key, "price"))
                row->has_price = 1;
            else if (ulab_streq(key, "validity"))
                row->has_validity = 1;
                    ...
                    ulab_error_t *err)
{
    ...
    *found = 0;
    ...
    for (i = 0; i < json_array_size(arr); i++)
    {
        ...
        release_version = value && json_is_string(value) ? json_string_value(value) : NULL;
                                 ...
                                 ulab_error_t *err)
{
    ...
    arr = softwares && json_is_object(softwares) ? json_object_get(softwares, "software") : NULL;
    if (arr == NULL || !json_is_array(arr))
    {
        snprintf(err->msg, sizeof(err->msg),
...
{
    if (node_type == NULL || node_type[0] == '\0')
        return "";
                                      ...
                                      ulab_error_t *err)
{
    ...
    if (ulab_streq(entity, "node") && ulab_streq(view, "site_detail"))
    {
        ...
        expected_site = expected_node ? world_site_by_ref(mutable_world, expected_node->site_ref) : NULL;
                                  ...
                                  ulab_error_t *err)
{
    ...
    *matched = 0;
             ...
             ulab_streq(entity, "plan"))
    {
        ...
        duration = expected && expected->duration_minutes > 0 ? (int)expected->duration_minutes : (expected ? (int)(expected->duration_days * 1440u) : 0);
        *matched = expected != NULL &&
