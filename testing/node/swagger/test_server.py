...
def create_mock_endpoint(endpoint, method, details):
    response_code, response_details = next(iter(details['responses'].items()))

    def handler():
        return jsonify({'message': response_details.get('description', 'Mock response')}), int(response_code)
...
def catch_all(path):
    return jsonify({'message': 'This is a mock server'}), 200
