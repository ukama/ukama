...
func (r *Hss) GetUsers(orgName string) (*pb.ListUsersResponse, error) {
	...
	return r.client.List(ctx, &pb.ListUsersRequest{
		Org: orgName,
	})
}
...
func (r *Hss) Delete(userId string) error {
	...
	_, err := r.client.Delete(ctx, &pb.DeleteUserRequest{Uuid: userId})
	return err
}

func (r *Hss) Get(userId string) (*pb.GetUserResponse, error) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(r.timeout)*time.Second)
	defer cancel()

	return r.client.Get(ctx, &pb.GetUserRequest{Uuid: userId})
}
