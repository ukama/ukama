  ...
  return (
    <Box component="div" sx={{ height: 'calc(100% - 8%)' }}>
      <LoadingWrapper
        width="100%"
        height="inherit"
        isLoading={isSkeltonLoad || usersByOrgLoading || updateUserLoading}
      >
        {usersRes && usersRes?.getUsersByOrg?.length > 0 ? (
          <RoundedCard sx={{ borderRadius: '4px', overflow: 'hidden' }}>
            <ContainerHeader
              title="My Users"
              showButton={true}
              showSearchBox={false}
                ...
                <Grid key={item.id} item xs={12} sm={6} md={4} lg={3}>
                  ...
                  >
                    <UserCard
                      ...
                      handleMoreUserdetails={onViewMoreClick}
                    />
                  </Card>
                </Grid>
              ))}
        ...
        {simDialog.isShow && (
          <UserDetailsDialog
            ...
            saveBtnLabel={'Save'}
            closeBtnLabel="close"
            loading={userLoading || addUserLoading || updateUserLoading}
            isOpen={true}
