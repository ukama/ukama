...
import EditSiteDialog from '@/app/(main)/console/sites/[id]/_components/EditSiteDialog';
import LoadingWrapper from '@/components/ui/LoadingWrapper';
import SitesWrapper from '@/app/(main)/console/sites/_components/SitesWrapper';
...
export default function Page() {
  ...
  const { subscribe, unsubscribe: unsubscribeManager } = useSubscriptionManager();
      ...
      onCompleted: (data) => {
        if (data.getSiteStat.metrics.length > 0) {
          ...
          if (!subscriptionsRef.current[sKey]) {
            ...
            subscribe(sKey, () => { PubSub.unsubscribe(token); });
          }
        }
      },
    });
