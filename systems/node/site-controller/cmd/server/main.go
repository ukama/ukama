...
func runGrpcServer(gormdb sql.Db) {
	...
	mbClient := mb.NewMsgBusClient(svcConf.MsgClient.Timeout,
		svcConf.OrgName,
		pkg.SystemName,
		pkg.ServiceName,
		instanceId,
	...
	cmdAdapter, err := adapters.NewControllerAdapter(svcConf.Services.Controller,
		svcConf.Services.Timeout)
	if err != nil {
		log.Fatalf("failed to connect controller: %v", err)
	}

	r := reconciler.New(db.NewIntentRepo(gormdb),
	...
	siteServer := server.NewSiteControllerServer(r)
	eventServer := server.NewSiteControllerEventServer(svcConf.OrgName,
		siteServer,
		mbClient)

	grpcServer := ugrpc.NewGrpcServer(*svcConf.Grpc, func(s *grpc.Server) {
		pb.RegisterSiteControllerServiceServer(s, siteServer)
...
func msgBusListener(m mb.MsgBusServiceClient) {
	if err := m.Register(); err != nil {
		log.Fatalf("Failed to register to Message Client Service. Error %s",
			err.Error())
	}

	if err := m.Start(); err != nil {
		log.Fatalf("Failed to start Message Client Service routine for service %s. Error %s",
			pkg.ServiceName,
			err.Error())
	}
}
