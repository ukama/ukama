...
import (
	...
	"github.com/ukama/ukama/systems/node/operation-monitor/pkg/server"

	"github.com/ukama/ukama/systems/node/operation-monitor/cmd/version"
)
...
func initDb() sql.Db {
	...
	if err := d.Init(&db.MonitoredIntent{}); err != nil {
...
func runGrpcServer(gormDb sql.Db) {
	...
	repo := db.NewIntentRepo(gormDb)
	monitor := server.NewMonitorServer(svcConf.OrgName, svcConf.OrgId, repo, mbClient)
	eventServer := server.NewEventServer(svcConf.OrgName, monitor)

	grpcSrv := ugrpc.NewGrpcServer(*svcConf.Grpc, func(s *grpc.Server) {
		pb.RegisterOperationMonitorServiceServer(s, monitor)
		egenerated.RegisterEventNotificationServiceServer(s, eventServer)
	})

	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	go server.NewSweeper(monitor).Run(ctx)
