...
func (r *Router) init(f func(*gin.Context, string) error) {
	...
	if r.config.grpcEndpoints != nil {
		rest.RegisterStatusEndpoint(r.f, pkg.SystemName, map[string]rest.StatusTarget{
			"package":  {Host: r.config.grpcEndpoints.Package, Description: desc.Package},
			"baserate": {Host: r.config.grpcEndpoints.Baserate, Description: desc.Baserate},
			"rate":     {Host: r.config.grpcEndpoints.Rate, Description: desc.Rate},
...
func (r *Router) getBaseRatesHandler(c *gin.Context, req *GetBaseRatesRequest) (*bpb.GetBaseRatesResponse, error) {
	return r.clients.b.GetBaseRates(&bpb.GetBaseRatesRequest{
		Country:  req.Country,
