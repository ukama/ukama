...
// IsPackageNameAvailable provides a mock function with given fields: name
func (_m *packageS) IsPackageNameAvailable(name string) (*gen.IsNameAvailableResponse, error) {
	ret := _m.Called(name)

	if len(ret) == 0 {
		panic("no return value specified for IsPackageNameAvailable")
	}

	var r0 *gen.IsNameAvailableResponse
	var r1 error
	if rf, ok := ret.Get(0).(func(string) (*gen.IsNameAvailableResponse, error)); ok {
		return rf(name)
	}
	if rf, ok := ret.Get(0).(func(string) *gen.IsNameAvailableResponse); ok {
		r0 = rf(name)
	} else {
		if ret.Get(0) != nil {
