 ...
 * source:     services, device
 * container:  service_name
 * object:     general, link, cert
 * state:      register, unregister, connect, fail, active, lost, 
 *             end, close, valid, invalid, update, expired
 */
...
static int is_valid_event(MsgClientEvent event);
static char *create_routing_key(MsgClientEvent event, char *name);
static int object_type(MsgClientEvent event);

/* Mapping between various event and AMQP routing key
 *
 * event             object   state      type
 * --------------------------------------------
 * SRVC_REGISTER      general  register   event
 * SRVC_UNREGISTER    general  unregister event
 *
...
static AMQPRoutingKey routingKey[] = {
	...
	[SRVC_REGISTER]   = {.type=EVENT, .object=GENERAL, .state=REGISTER},
	[SRVC_UNREGISTER] = {.type=EVENT, .object=GENERAL, .state=UNREGISTER},
...
static char *convert_state_to_str(ObjectState state) {
	...
	case REGISTER:
		str = STATE_REGISTER_STR;
		break;

	case UNREGISTER:
		str = STATE_UNREGISTER_STR;
		break;
...
/*
 * init_amqp_connection -- intialize connection to host:port using AMQP
 *
 */
...
/*
 * close_amqp_connection -- close an established AMQP connection
 *
 */
...
static char *create_routing_key(MsgClientEvent event, char *name) {
	...
	source = convert_source_to_str((MsgSource)SERVICE);

	len = strlen(type) + strlen(source) + strlen(name) + strlen(object)
		+ strlen(state);
...
int publish_amqp_event(WAMQPConn *conn, char *exchange,
	 ...
	 * <type>.<source>.<container>.<object>.<state>
	 */
	key = create_routing_key(event, name);
	if (key == NULL) {
