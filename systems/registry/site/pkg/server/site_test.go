...
func TestSiteService_Get(t *testing.T) {
	...
	s := NewSiteServer(OrgName, siteRepo, msgclientRepo, netRepo, "", nil, nil, nil)
...
func TestSiteService_Update(t *testing.T) {
	...
	t.Run("NilMessageBus", func(t *testing.T) {
		...
		sNoMsgBus := NewSiteServer(OrgName, siteRepo, nil, netRepo, "", nil, nil, nil)
...
func TestSiteService_Add(t *testing.T) {
	for _, failure := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			repo := &mocks.SiteRepo{}
			networks := &mocks.NetworkClientProvider{}
			network := &netmocks.NetworkServiceClient{}
			inventoryClient := &cmocks.ComponentClient{}
			networks.On("GetClient").Return(network, nil)
			network.On("Get", mock.Anything, mock.Anything).Return(&npb.GetResponse{}, nil)
			tower := ukama.NewVirtualTowerNodeId().String()
			for _, id := range []string{testBackhaulId.String(), testPowerId.String(), testAccessId.String(), testSwitchId.String(), testSpectrumId.String()} {
				inventoryClient.On("Get", id).Return(&inventory.ComponentInfo{PartNumber: tower}, nil)
			}
			server := NewSiteServer(OrgName, repo, nil, networks, "", inventoryClient, nil, nil)
			store := &memoryProvisions{}
			store.begin = func(site *db.Site, nodes []string) (*db.SiteProvision, error) {
				assert.Equal(t, 3, len(nodes))
				assert.Equal(t, tower, nodes[0])
				assert.NotEqual(t, nodes[0], nodes[1])
				assert.NotEqual(t, nodes[0], nodes[2])
				if failure {
					return nil, errors.New("configuration failed")
				}
				return &db.SiteProvision{Site: *site, Phase: "active"}, nil
			}
			server.provisions = store
			req := createValidAddRequest()
			response, err := server.Add(context.Background(), req)
			if failure {
				assert.Error(t, err)
				...
				verifyAddRequestFields(t, req, response.Site)
			}
			// Add does not directly bypass the durable coordinator.
			repo.AssertNotCalled(t, "Add", mock.Anything, mock.Anything)
			inventoryClient.AssertExpectations(t)
		})
	}
	t.Run("InvalidNetwork", func(t *testing.T) {
		server := NewSiteServer(OrgName, nil, nil, nil, "", nil, nil, nil)
		req := createValidAddRequest()
		req.NetworkId = "invalid"
