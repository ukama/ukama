...
func NewNodeServer(nodeRepo db.NodeRepo, org string, pushGatewayHost string) *NodeServer {
...
func (n *NodeServer) UpdateNodeState(ctx context.Context, req *pb.UpdateNodeStateRequest) (*pb.UpdateNodeStateResponse, error) {
	...
	if dbState == db.Onboarded {
		err = pmetric.CollectAndPushSimMetrics(n.pushGatewayHost, pkg.NodeMetric, pkg.NumberOfActiveNodes, float64(activeNodeCount), map[string]string{"network": "", "org": n.org}, pkg.SystemName)
		if err != nil {
			logrus.Errorf("Error while pushing subscriberCount metric to pushgaway %s", err.Error())
		}
	} else if dbState == db.Pending {
		err = pmetric.CollectAndPushSimMetrics(n.pushGatewayHost, pkg.NodeMetric, pkg.NumberOfInactiveNodes, float64(inactiveNodeCount), map[string]string{"network": "", "org": n.org}, pkg.SystemName)
		if err != nil {
			logrus.Errorf("Error while pushing subscriberCount metric to pushgaway %s", err.Error())
		}
...
func (n *NodeServer) AddNode(ctx context.Context, req *pb.AddNodeRequest) (*pb.AddNodeResponse, error) {
	...
	err = pmetric.CollectAndPushSimMetrics(n.pushGatewayHost, pkg.NodeMetric, pkg.NumberOfNodes, float64(nodesCount), map[string]string{"network": "", "org": n.org}, pkg.SystemName)
	if err != nil {
		logrus.Errorf("Error while pushing subscriberCount metric to pushgaway %s", err.Error())
	}
