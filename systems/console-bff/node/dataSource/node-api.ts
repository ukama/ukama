...
class NodeAPI extends RESTDataSource {
  ...
  async getNode(args: NodeInput): Promise<Node> {
    return this.get(`/v1/nodes/${args.id}`).then(res => parseNodeRes(res.node));
  }
  async getNodes(args: boolean): Promise<GetNodes> {
    return this.get(`/v1/nodes?free=${args}`).then(res => parseNodesRes(res));
  }
  async deleteNodeFromOrg(args: NodeInput): Promise<DeleteNode> {
    return this.delete(`/v1/nodes/${args.id}/sites`).then(() =>
      this.delete(`/v1/nodes/${args.id}`).then(() => {
        return { id: args.id };
      })
    );
  }
  async attachNode(args: AttachNodeInput): Promise<CBooleanResponse> {
    return this.post(`/v1/nodes/${args.parentNode}/attach`, {
      body: {
        anodel: args.anodel,
        anoder: args.anoder,
      },
  ...
  async detachhNode(args: NodeInput): Promise<CBooleanResponse> {
    return this.delete(`/v1/nodes/${args.id}/detach`).then(res =>
      res ? { success: true } : { success: false }
    );
  }
  async addNode(args: AddNodeInput): Promise<Node> {
    return this.post(`/v1/nodes`, {
      body: {
        name: args.name,
        node_id: args.id,
        org_id: args.orgId,
  ...
  async addNodeToSite(args: AddNodeToSiteInput): Promise<CBooleanResponse> {
    return this.post(`/v1/nodes/${args.nodeId}/sites`, {
      body: {
  ...
  async releaseNodeFromSite(args: NodeInput): Promise<CBooleanResponse> {
    return await this.delete(`/v1/nodes/${args.id}/sites`).then(res =>
      res ? { success: true } : { success: false }
    );
  }
  async updateNodeState(args: UpdateNodeStateInput): Promise<Node> {
    return this.patch(`/v1/nodes/${args.id}/state/${args.state}`).then(res =>
      parseNodeRes(res)
    );
  }
  async updateNode(args: UpdateNodeInput): Promise<Node> {
    return this.put(`/v1/nodes/${args.id}`, {
      body: {
        name: args.name,
      },
    }).then(res => parseNodeRes(res.node));
