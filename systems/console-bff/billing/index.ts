...
import { findProcessNKill, parseHeaders } from "../common/utils";
...
const runServer = async () => {
  const isSuccess = await findProcessNKill(`${BILLING_PORT}`);
  if (isSuccess) {
    ...
    logger.error(`Server failed to start on port ${BILLING_PORT}`);
  }
};
