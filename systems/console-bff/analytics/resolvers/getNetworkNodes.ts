...
import { NetworkNodesDto } from "./network.types";
import { AnalyticsWindowInput } from "./shared";

@Resolver()
export class GetNetworkNodesResolver {
  @Query(() => NetworkNodesDto)
  async getNetworkNodes(
    @Arg("data") data: AnalyticsWindowInput,
    @Ctx() ctx: AppContext
  ): Promise<NetworkNodesDto> {
    const baseURL = await ctx.urls.url("analytics");
    return ctx.dataSources.analytics.getNetworkNodes(baseURL, data);
  }
}
