...
// GetEmailByExternalRef provides a mock function with given fields: externalRef
func (_m *MailerRepo) GetEmailByExternalRef(externalRef string) (*db.Mailing, error) {
	ret := _m.Called(externalRef)

	if len(ret) == 0 {
		panic("no return value specified for GetEmailByExternalRef")
	}

	var r0 *db.Mailing
	var r1 error
	if rf, ok := ret.Get(0).(func(string) (*db.Mailing, error)); ok {
		return rf(externalRef)
	}
