...
func (r *userNotificationRepo) GetNotificationsByUserID(id string) ([]*Notifications, error) {
	...
	q := fmt.Sprintf("SELECT user_notifications.is_read,notifications.title,notifications.is_process_required,notifications.resource_id,notifications.description,notifications.scope,notifications.type,notifications.id,notifications.created_at,notifications.updated_at FROM user_notifications INNER JOIN notifications ON user_notifications.notification_id = notifications.id WHERE user_notifications.user_id = '%s';", id)
