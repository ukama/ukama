...
func init() {
	tConfig = &TestConfig{
		HubNodegateway: "http://node-gateway:8080",
	}
...
func Test_AddCApp(t *testing.T) {
	...
	appUrl := fmt.Sprintf("%s%s/%s/%s", tConfig.HubNodegateway, AppsPath, AppName, AppVer)
	con := getFileContent(t)
	rest := resty.New().EnableTrace().SetDebug(tConfig.DebugMode)

	t.Run("Ping", func(t *testing.T) {
		r, err := rest.R().Get(tConfig.HubNodegateway + "/ping")
