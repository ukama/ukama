...
type Profile struct {
	...
	Iccid string `gorm:"index:profile_iccid_idx,unique,where:deleted_at is null;not null;size:22;check:iccid_checker,iccid ~ $$^\\d+$$"`
	...
	Imsi string `gorm:"index:profile_imsi_idx,unique,where:deleted_at is null;not null;size:15;check:imsi_checker,imsi ~ $$^\\d+$$"`
	...
	ConsumedDataBytes       uint64
	LastStatusChangeAt      time.Time
	LastStatusChangeReasons StatusReason // Hold the reason for last status change which is for activation or deactivation
}
