        ...
        if (data->temperature_c > cfg->highTempCrit) {
            ...
            if (checker->high_temp_count == checker->debounce_samples) {
                ...
                snprintf(msg, sizeof(msg),
                         "Controller temperature critical: %.1f°C > %.1f°C",
            ...
            if (checker->high_temp_count == checker->debounce_samples) {
                char msg[128];
                snprintf(msg, sizeof(msg),
                         "Controller temperature high: %.1f°C > %.1f°C",
...
void alarms_check_comm_failure(AlarmChecker *checker, bool comm_ok) {
    ...
    if (!comm_ok) {
        ...
        if (checker->comm_fail_count == checker->debounce_samples) {
            ...
            metrics_store_set_alarm(checker->store,
                                    ALARM_COMMUNICATION_LOST,
                                    SEVERITY_CRITICAL,
                                    msg);
            alarms_send_notification(checker->config,
                                     ALARM_COMMUNICATION_LOST,
                                     SEVERITY_CRITICAL,
                                     msg);
        }
