...
import (
	...
	datapath "github.com/ukama/ukama/nodes/apps/pcrf/pkg/datapath"
)
...
func (_m *DataPath) AddNewDataPath(ip string, rxMeter uint32, txMeter uint32, rxRate uint32, txRate uint32, burstSize uint32, rxCookie uint64, txCookie uint64) error {
	...
	if len(ret) == 0 {
		panic("no return value specified for AddNewDataPath")
	}
...
// DataPathCount provides a mock function with no fields
func (_m *DataPath) DataPathCount() uint32 {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for DataPathCount")
	}
...
func (_m *DataPath) DataPathStats(rxCookieID uint64, txCookieID uint64) (uint64, uint64, uint64, uint64, error) {
	...
	if len(ret) == 0 {
		panic("no return value specified for DataPathStats")
	}
...
func (_m *DataPath) DeleteDataPath(ip string, rxMeter uint32, txMeter uint32) error {
	...
	if len(ret) == 0 {
		panic("no return value specified for DeleteDataPath")
	}
...
func (_m *DataPath) Status() datapath.Status {
	ret := _m.Called()
	...
	var r0 datapath.Status
	if rf, ok := ret.Get(0).(func() datapath.Status); ok {
		r0 = rf()
	} else {
		r0 = ret.Get(0).(datapath.Status)
	}
