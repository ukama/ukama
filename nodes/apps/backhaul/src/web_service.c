...
static void setup_unsupported_methods(UInst *instance, char *allowedMethod, char *prefix, char *resource) {
...
int start_web_service(Config *config, UInst *serviceInst, EpCtx *epCtx) {
	...
	if (ulfius_init_instance(serviceInst, config->servicePort, NULL, NULL) != U_OK) {
		usys_log_error("Error initializing instance for webservice port %d", config->servicePort);
		return USYS_FALSE;
	}
	...
	if (ulfius_start_framework(serviceInst) != U_OK) {
		usys_log_error("Error starting the webservice.");
...
int web_service_cb_status(const URequest *request, UResponse *response, void *epConfig) {
	...
	json_t *o = metrics_store_status_json(ctx->store);
...
int web_service_cb_metrics(const URequest *request, UResponse *response, void *epConfig) {
	...
	EpCtx *ctx = (EpCtx *)epConfig;
	json_t *o = metrics_store_snapshot_json(ctx->store);
	respond_json(response, HttpStatus_OK, o);
	json_decref(o);
	return U_CALLBACK_CONTINUE;
}
...
int web_service_cb_post_diag_chg(const URequest *request, UResponse *response, void *epConfig) {
	...
	EpCtx *ctx = (EpCtx *)epConfig;
	worker_enqueue(ctx->worker, JOB_CHG);
	metrics_store_set_diag(ctx->store, "chg");
...
int web_service_cb_post_diag_parallel(const URequest *request, UResponse *response, void *epConfig) {
	...
	EpCtx *ctx = (EpCtx *)epConfig;
	worker_enqueue(ctx->worker, JOB_DIAG_PARALLEL);
	metrics_store_set_diag(ctx->store, "parallel");
...
int web_service_cb_post_diag_bufferbloat(const URequest *request, UResponse *response, void *epConfig) {
	...
	EpCtx *ctx = (EpCtx *)epConfig;
	worker_enqueue(ctx->worker, JOB_DIAG_BUFFERBLOAT);
	metrics_store_set_diag(ctx->store, "bufferbloat");
