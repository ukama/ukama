...
void classifier_run(Config *config, MetricsStore *store) {
	...
	/* compute percentiles for near/far from buffers (reuse helper in metrics_store.c by quick inline) */
	/* minimal v1: use last computed values if any exist; else derive rough from last samples */
	double nearP95 = store->metrics.nearTtfbP95Ms;
	double farMed  = store->metrics.farTtfbMedianMs;

	/* basic health from recent micro probes */
	int recentFails = count_recent_failures_locked(store, config->downConsecFails);

	/* state machine: DOWN vs recover */
	...
	/* degradation: stalls or high tail latency */
	/* we approximate using last stored stallRatePct if available */
	...
	/* if far is extremely higher than near, bump satellite confidence slightly */
	...
	/* if we have cap, treat guess as unknown unless latency strongly indicates sat */
