...
static JsonObj *build_status_payload(RawRs485Context *ctx)
{
    ...
    if (ctx != NULL) {
        json_object_set_new(json, "address", json_integer(ctx->device.address));
        json_object_set_new(json, "linked", json_boolean(ctx->device.linked));
        json_object_set_new(json, "baud", json_integer(ctx->device.baud));
        json_object_set_new(json, "deviceType", json_integer(ctx->device.deviceType));
        json_object_set_new(json, "uniqueId", json_string(ctx->device.uniqueIdHex));
        json_object_set_new(json, "model", json_string(ctx->device.model));
    }
