...
static bool setup_tun(Config *config, AppStatus *status) {
    ...
    exec_cmd(config->cmdTimeoutSec, "ip", "link", "delete", config->tunIf, NULL);
    ...
    {
        char rpFilter[OVS_MAX_STR];
        char acceptLocal[OVS_MAX_STR];
        char logMartians[OVS_MAX_STR];

        snprintf(rpFilter, sizeof(rpFilter),
                 "net.ipv4.conf.%s.rp_filter=2", config->tunIf);
        snprintf(acceptLocal, sizeof(acceptLocal),
                 "net.ipv4.conf.%s.accept_local=1", config->tunIf);
        snprintf(logMartians, sizeof(logMartians),
                 "net.ipv4.conf.%s.log_martians=1", config->tunIf);

        exec_cmd(config->cmdTimeoutSec, "sysctl", "-w", rpFilter, NULL);
        exec_cmd(config->cmdTimeoutSec, "sysctl", "-w", acceptLocal, NULL);
        exec_cmd(config->cmdTimeoutSec, "sysctl", "-w", logMartians, NULL);
    }
...
static bool setup_flows(Config *config, AppStatus *status) {
    ...
    if (config->defaultDrop) {
        snprintf(srcDrop, sizeof(srcDrop), "priority=10,ip,nw_src=%s,actions=drop",
                 config->ueCidr);
        snprintf(dstDrop, sizeof(dstDrop), "priority=10,ip,nw_dst=%s,actions=drop",
                 config->ueCidr);
...
bool ovs_setup(Config *config, AppStatus *status) {
    ...
    /*
     * TODO:
     * - Add EPC-emu user-plane veth/tun attachment into br0 once EPC-emu
     *   exposes a stable interface contract.
     * - Add media/forwarder container interface attachment into br0.
     * - Add table-1000/table-2000 policy routing once UE and media paths are
     *   finalized for virtual-node E2E.
     * - Make stale PCRF flow reconciliation explicit if PCRF owns.
     */

    status_set(status, InitStateReady, "none");

    usys_log_info("network ready bridge=%s ue=%s", config->bridge,
                  config->ueCidr);
