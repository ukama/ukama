...
json_t *json_serdes_power_metrics_to_json(const PowerMetrics *m) {
    ...
    json_object_set_new(o, "sampleUnixMs",
                        json_integer((json_int_t)m->sampleUnixMs));
    ...
    json_object_set_new(o, "severity", json_string(m->severity));
    json_object_set_new(o, "reason", json_string(m->reason));
    json_object_set_new(o, "totalWatts", json_real(m->totalWatts));
    json_object_set_new(o, "energyWh", json_real(m->energyWh));
    ...
    if (m->haveLm25066) {
        ...
        json_object_set_new(p, "assumedDirect", json_boolean(m->assumedDirect));
