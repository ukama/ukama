...
static int ber_get_len(const uint8_t *buf,
                       size_t bufLen,
                       size_t *len,
                       size_t *used) {
...
static int ber_put_tlv(uint8_t type,
                       const uint8_t *val,
                       size_t vlen,
                       uint8_t *out,
                       size_t outLen,
...
static int ber_encode_int(int64_t value,
                          uint8_t *buf,
                          size_t bufLen,
                          size_t *written) {
    ...
    return ber_put_tlv(ASN_INTEGER,
                       tmp + i,
                       sizeof(tmp) - i,
                       buf,
                       bufLen,
...
static int ber_encode_string(uint8_t type,
                             const void *data,
                             size_t len,
                             uint8_t *buf,
                             size_t bufLen,
...
static int ber_encode_oid(const uint32_t *oid,
                          size_t oidLen,
                          uint8_t *buf,
                          size_t bufLen,
                          size_t *written) {
...
static int ber_decode_oid(const uint8_t *buf,
                          size_t len,
                          uint32_t *oid,
                          size_t *oidLen) {
...
static int decode_value(uint8_t type,
                        const uint8_t *buf,
                        size_t len,
                        SnmpValue *out) {
    ...
    switch (type) {
    case ASN_INTEGER:
        if (ber_decode_int(buf, len, &iv) != 0) {
            return -1;
        }
    ...
    case ASN_OPAQUE:
        ...
        if (len >= sizeof(out->stringValue)) {
            len = sizeof(out->stringValue) - 1;
        }
    ...
    case ASN_OBJECT_ID:
        ...
        if (ber_decode_oid(buf, len, out->oid, &olen) != 0) {
            return -1;
        }
...
static int build_request(int pduType,
    ...
    if (setVal == NULL || setVal->type == SNMP_VALUE_NONE) {
        ...
        if (ber_encode_int(setVal->intValue,
                           valTlv,
                           sizeof(valTlv),
                           &nVal) != 0) {
            return -1;
...
static int parse_response(const uint8_t *buf,
                          size_t len,
                          int32_t expectReqId,
                          SnmpVarBind *out) {
    ...
    if (len < 2 || buf[off++] != ASN_SEQUENCE) {
        return -1;
    }

    if (ber_get_len(buf + off, len - off, &l, &used) != 0) {
        return -1;
    }
    ...
    if (ber_decode_int(buf + off, l, &iv) != 0 || iv != expectReqId) {
        return -1;
    }
    ...
    if (ber_decode_int(buf + off, l, &iv) != 0 || iv != 0) {
        return -1;
    }
    ...
    if (ber_decode_oid(buf + off, l, oid, &oidLen) != 0) {
        return -1;
    }
    ...
    if (out != NULL) {
        return decode_value(type, buf + off, l, &out->value);
    }
...
int snmp_session_init(SnmpSession *s,
    ...
    if (s == NULL || host == NULL || community == NULL) {
...
static void snmp_oid_debug(const uint32_t *oid,
                           size_t oidLen,
                           char *buf,
                           size_t bufLen) {
...
static int snmp_request(SnmpSession *s,
                        int pduType,
                        const uint32_t *oid,
                        size_t oidLen,
                        const SnmpValue *setVal,
...
int snmp_walk(SnmpSession *s,
    ...
    for (;;) {
        ...
        if (!snmp_oid_has_prefix(vb.oid, vb.oidLen, baseOid, baseLen)) {
            return SWITCHD_OK;
        }
...
int snmp_oid_from_string(const char *s, uint32_t *oid, size_t *oidLen) {
    ...
    if (s == NULL || oid == NULL || oidLen == NULL) {
        return -1;
    }
