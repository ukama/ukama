...
void *_dmt_alloc(size_t sz, int zeroset, const char *file, unsigned line) {
  ...
  if (node == NULL) {
    ...
    log_error("Couldn't allocate: %s, line %u\n", file, line);
    _dmt_abort();
#else
    return NULL;
#endif
...
void *_dmt_realloc(void *ptr, size_t sz, const char *file, unsigned line) {
  ...
  if (!_dmt_has_node(node)) {
    log_error("Bad realloc: %p %s, line %u\n", ptr, file, line);
    _dmt_abort();
  }
#endif
  ...
  if (node == NULL) {
#ifdef DMT_ABORT_NULL
    log_error("Couldn't reallocate: %s, line %u\n", file, line);
    _dmt_abort();
#else
    return NULL;
#endif
...
void dmt_dump(void) {
  ...
  while (node != NULL) {
    log_info("Unfreed: %p %s, line %lu (%lu bytes)\n", 
  ...
  log_info("Total unfreed: %lu bytes\n", (unsigned long)total);
}
...
#ifndef DMT_UNSAFE
  if (!_dmt_has_node(node)) {
    log_error("Bad pointer: %p %s, line %u\n", ptr, file, line);
    _dmt_abort();
  }
#endif
