...
static void ready_touch(Config *config) {
    ...
    if (!config || !config->readyFile) return;
...
static char* app_exec_path(Config *config, App *app) {
    ...
    if (!config || !app || !app->cmd) return NULL;

    if (app->cmd[0] == '/') return strdup(app->cmd);
        ...
        while (sp) {
            ...
            while (a) {
                if (a->pid == pid) {
                    ...
                    if (a->state == APP_STATE_STOPPING || a->state == APP_STATE_STOPPED) {
                    ...
                    usys_log_error("app: exited %s/%s restart in %d sec", a->space, a->name, delay);
                    sleep(delay);
...
static bool run_space(Config *config,
    ...
    while (a) {
        ...
        installer_switch_current(config, a);
...
static bool update_app(Supervisor *s,
    ...
    if (!s || !space || !name || !tag) return false;
...
static void* supervisor_thread(void *arg) {
    ...
    while (s->running) {
        ...
        if (a->type == ACTION_RUN_BOOT) {
            ...
            update_app(s, a->space, a->name, a->tag);
        }
...
Supervisor* supervisor_start(Config *config, Space *spaceList, ActionQueue *queue) {
    ...
    if (!config || !spaceList || !queue) return NULL;
