...
func DeleteFlowForUe(a *OfActor, ipString string, meter uint32) error {
	ip := net.ParseIP(ipString)
	if ip == nil {
		log.Errorf("Invalid IP address")
		return fmt.Errorf("invalid ip %s", ipString)
	...
	// openflow15 prtocol to delete flows
	...
	// if err != nil {
	// 	log.Errorf("failed to delete flow for UE %s and meter %d. Error: %s", ipString, meter, err.Error())
	// 	return err
	// }

	// err = updateFlowForUe(a, ipString, meter, openflow15.FC_DELETE)
	// if err != nil {
	// 	log.Errorf("failed to delete flow for UE %s and meter %d. Error: %s", ipString, meter, err.Error())
	// 	return err
	// }

	// log.Infof("Deleted flow for UE %s and meter %d.", ipString, meter)
	// return nil

	flow := new(ofctrl.Flow)
	flow.Table = a.inputTable
	flow.Match = ofctrl.FlowMatch{
		Ethertype: 0x0800,
		Priority:  100,
		IpSa:      &ip,
	}
	flow.UpdateInstallStatus(true)
	// See if the flow already exists
	// flowKey := GetFlowKey(flow.Match)
	// log.Infof("Flow key is %s", flowKey)
...
func start() {
	...
	err = DeleteFlowForUe(sw.ofActor, "192.168.8.3", id)
	if err != nil {
		log.Errorf("Failed to add  flow.")
	}
