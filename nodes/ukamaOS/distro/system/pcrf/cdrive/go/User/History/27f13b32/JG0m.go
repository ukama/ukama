...
*/
func createFlowForUe(a *OfActor, ipString string, meter uint32) error {
	...
	rxF.ApplyAction(ma)
	rxF.ApplyAction(normalAction)
	err = rxF.Send(openflow15.FC_DELETE)
	if err != nil {
...
func deleteFlowForUe(a *OfActor, ipString string, meter uint32) error {
	f, err := createFlowForUe(a, ipString, meter) {
		fi
	}
