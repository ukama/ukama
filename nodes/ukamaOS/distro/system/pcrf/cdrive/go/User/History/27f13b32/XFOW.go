...
func dumpflowStats(a *OfActor, ipString string) error {
	...
	var cookieID uint64 = 0x1000000001001
	cookieMask := uint64(0xffffff)
	stats, err := a.Switch.DumpFlowStats(cookieID, &cookieMask, &f.Match, &f.Table.TableId)
	if err != nil {
...
func start() {
	...
	err = AddFlowForUe(sw.ofActor, "192.168.8.4", id)
	if err != nil {
		log.Errorf("Failed to add  flow.")
	}

	err = dumpflowStats(sw.ofActor, "192.168.8.4")

	err = DeleteFlowForUe(sw.ofActor, "192.168.8.4")
	if err != nil {
		log.Errorf("Failed to add  flow.")
	}
