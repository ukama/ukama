...
int verify_uuid(char* uuid) {
	...
	if ((!len) || (len > UUID_MAX_LENGTH)) {
		log_error("MFG:: Error:: UUID length should be greater than 0 and less than 24 characters.");
		ret = -1;
	}
	return ret;
}
...
int verify_boardname(char* name) {
	...
	if ((!len) || (len > UUID_MAX_LENGTH)) {
		log_error("MFG:: Error:: Name length should be greater than 0 and less than 24 characters.");
	} else {
		...
		if (ret) {
			log_error("MFG:: Error:: Check the module name %s.", name);
		}
