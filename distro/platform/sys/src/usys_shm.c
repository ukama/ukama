...
int usys_free_shared_mem(const char *name, void *ptr, uint32_t size) {
  ...
  if ((ret = usys_munmap(ptr, size)) < 0) {
    usys_log_error("Could not munmap shared memory %s, ptr %p. Error: %s", name,
                   ptr, usys_error(errno));
    return ret;
  }
...
void *usys_map_shared_mem(const char *name, uint32_t size) {
  ...
  if ((mem = usys_mmap(0, size, PROT_EXEC | PROT_READ | PROT_WRITE, MAP_SHARED,
                       fd, 0)) == MAP_FAILED) {
