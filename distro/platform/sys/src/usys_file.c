...
int usys_file_remove(void *data) {
  ...
  if (data) {
    ...
    if (!ret) {
      ...
      usys_log_debug("File %s not deleted. Error %s", fname, usys_error(errno));
    }
  }
  return ret;
}
...
int usys_file_symlink_exists(const char *path) {
  ...
  if (usys_lstat(path, &sb) == 0) {
    ...
    usys_log_error("Failed to get info from symbolic link %s. Error: %s", path,
                   usys_error(errno));
  }
  return (ret);
}
...
char *usys_file_read_sym_link(char *fname) {
  ...
  if (usys_lstat(fname, &sb) == -1) {
    usys_log_error("Failed to get info from symbolic link %s. Error: %s", fname,
                   usys_error(errno));
    return NULL;
  }
