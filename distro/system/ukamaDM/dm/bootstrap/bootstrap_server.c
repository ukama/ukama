...
/* tell whether the endpoint contains asterisk character. */
static bool prv_ep_name_contains_asterisk(char * buff)
{
    int i = 0;
    while (buff != NULL && buff[i] != '\0')
    {
        if (buff[i] == '*')
            return true;
...
static bool prv_ep_asterisk_name_compare(char *buff, char *comparewith)
{
    int i = 0;
    bool mismatch = false;

    while (buff != NULL && buff[i] != '\0' && comparewith != NULL && comparewith[i] != '\0')
    {
        if (comparewith[i] != '*' && buff[i] != comparewith[i])
        {
            mismatch = true;
            return false;
        }
        else if (comparewith[i] == '*')
        {
            return true;
        }
    ...
    if (buff[i] == '\0' && comparewith[i] == '*' && !mismatch)
    {
        return true;
    }
                                  ...
                                  void * userData)
{
    ...
    case COAP_NO_ERROR:
    {
        ...
        while (endInfoP != NULL
            ...
            && !(
                  (
                      prv_ep_name_contains_asterisk(endInfoP->name)
                      && prv_ep_asterisk_name_compare(name, endInfoP->name)
                  )
                  ||
                  (
...
int main(int argc, char *argv[])
{
    ...
    print_bs_info(data.bsInfo);
    // free_bs_info(data.bsInfo);
    // print_bs_info(data.bsInfo);
    // return 1;
