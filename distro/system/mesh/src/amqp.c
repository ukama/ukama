...
static char *create_routing_key(MeshEvent event, int mode);
...
WAMQPConn *init_amqp_connection(char *host, char *port) {
  ...
  if (ret) {
    log_error("Unable to connect with AMQP server at host: %s port: %s", host,
	      port);
    // amqp_socket_delete(socket);
    amqp_destroy_connection(conn);
    return NULL;
  }
...
static char *create_routing_key(MeshEvent event, int mode) {
  ...
  if (mode == MODE_SERVER) {
    ...
    source = convert_source_to_str((MsgSource)DEVICE);
  }
...
int publish_amqp_event(WAMQPConn *conn, char *exchange, MeshEvent event,
   ...
   * <type>.<source>.<container>.<object>.<state>
   */
  key = create_routing_key(event, mode);
  if (key == NULL) {
  ...
  if (ret < 0) {
    ret = FALSE;
    log_error("Error sending AMQP message. Error: %s", amqp_error_string2(ret));
  } else {
    ret = TRUE;
